...
<hr size = "5" color = "green">
	...
	<%
			...
			for(BoardEntity brd : list) {
					...
					if ( ++row % 3 == 0) color = "white";
							
					else if (row % 3 == 1) color = "red";
			...
			<tr bgcolor = <%= color %>
				...
				<td align = center><a href = "editboard.jsp?id=<%= brd.getId() %>">
				<%= brd.getId()  %></a> <!-- editboard.jsp 호출할때 id값을 가지고 가라 -->
				<td align = left><%= brd.getTitle() %></td>
				<td align = center><%=brd.getName() %></td>
				<td align = center><%= brd.getAddr() %></td>
				
				
				<!--  게시 작성일을 2020-07-07 10:33:@1 형태로 출력 -->
